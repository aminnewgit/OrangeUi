...
import field_image from "@/lib/table/field_image";
import field_btn from "@/lib/table/field_btn.vue";
...
export default {
  ...
  props: {
    ...
    // 组件
    showSearchLine:{type:Boolean,default:true},
    rightBtn: Object,
  ...
  setup (props) {
    ...
    function getCellProps(field, scope){
      ...
      let show_value = filter ? filter(value, row, scope, field) : value
      show_value = (show_value === '' ||
                    show_value === undefined ||
                    show_value === null) ? '--' : show_value
      return {
        value:show_value,
        rawValue: value,
...
<template>
  <div v-if="showSearchLine" class="f ai-c jc-between c-table-top-btn-line">
    <div>
